[返回目录](./)

> 此页面正在建设中，当前为模板自动生成，部分信息可能不完整  
> 如果您愿意参与填写，可以[点此](https://github.dev/mkzi-nya/milthm-calculator-web)进行修改并提交，或联系@mkzi_nya(qq: 2450382239)
> 或[点击此处](info:download)下载对应文档并编辑后发送至[此群](https://qm.qq.com/q/3lwpuT3l8A)（qq：699731287）

# Moonflutter

![Moonflutter](../../jpgs/Moonflutter.jpg)

## 曲目信息
| 曲名 | [Moonflutter](info:info("Moonflutter")) |
| - | - |
| 曲师 | Reku Mochizuki |
| 曲绘 | nanakaria |
| 所属曲包 | 梦境磁带 |
| 更新版本 | v1.3 |
| 时长 | 03:02 |
| BPM | 87 |

## 谱面信息
| 难度 | [Drizzle](info:info("Moonflutter", "Drizzle")) | [Sprinkle](info:info("Moonflutter", "Sprinkle")) | [Cloudburst](info:info("Moonflutter", "Cloudburst")) | [Clear](info:info("Moonflutter", "Clear")) |
| - | - | - | - | - |
| 等级 | 3 (3.0) | 7+ (7.9) | 11 (11.4) | 11+ (11.7) |
| 谱师 | Kiyotsuki | Magaflutter | 云梦 % xzadudu179 | Papaflutter |
| Note数量 | 471 | 647 | 1422 | 1350 |

## 解锁方法

解锁曲包《[梦境磁带](./?q=dream_tape)》后，默认解锁。

## 曲目试听

- [【Youtube】](https://www.youtube.com/watch?v=54hs8Yhuffk)

## 曲目相关

- ver3.2 中曲绘发生了变化。

## 游戏相关

- CB 谱面在最后一段副歌前是难以读谱的谱面。左右两侧出现雨雾音符、正中间出现雨滴音符的地方，通过让雨雾音符来到边缘、同时按下雨雾音符与雨滴音符，即可用拇指（或双指）接下。
- CL 谱面自 ver6.0 起被视为隔离谱面，难度也从 11+ 变为 2*。内部值据说为 2.50*。
  - ver6.0 以前的定数为 11.7。

## 谱面预览

- [Drizzle](https://storage.mhtl.im/chart/Drizzle_Moonflutter.jpg)
- [Sprinkle](https://storage.mhtl.im/chart/Sprinkle_Moonflutter.jpg)
- [Cloudburst](https://storage.mhtl.im/chart/Cloudburst_Moonflutter.jpg)
- [Clear](https://storage.mhtl.im/chart/Clear_Moonflutter.jpg)

