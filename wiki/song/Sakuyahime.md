[返回目录](./)

> 此页面正在建设中，当前为模板自动生成，部分信息可能不完整  
> 如果您愿意参与填写，可以[点此](https://github.dev/mkzi-nya/milthm-calculator-web)进行修改并提交，或联系@mkzi_nya(qq: 2450382239)
> 或[点击此处](info:download)下载对应文档并编辑后发送至[此群](https://qm.qq.com/q/3lwpuT3l8A)（qq：699731287）

# Sakuyahime

![Sakuyahime](../../jpgs/Sakuyahime.jpg)

## 曲目信息
| 曲名 | [Sakuyahime](info:info("Sakuyahime")) |
| - | - |
| 曲师 | MYTK |
| 曲绘 | QIER |
| 所属曲包 | 花裳随雨得春迟 |
| 更新版本 | v4.0 |
| 时长 | 02:52 |
| BPM | 148 |

## 谱面信息
| 难度 | [Drizzle](info:info("Sakuyahime", "Drizzle")) | [Sprinkle](info:info("Sakuyahime", "Sprinkle")) | [Cloudburst](info:info("Sakuyahime", "Cloudburst")) |
| - | - | - | - |
| 等级 | 3+ (3.5) | 5+ (5.8) | 11+ (11.9) |
| 谱师 | Kiyotsuki | Kiyotsuki | Kiyotsuki |
| Note数量 | 483 | 1050 | 1480 |

## 解锁方法

- 《[灯ノ桜蝶](info:info("灯ノ桜蝶"))》的任意难度达成 C 及以上评级。

## 曲目试听

- [【Youtube】](https://youtu.be/KRjJ0iB8MS0)
- [【Bilibili】](https://b23.tv/AeBEULK)

## 曲目相关

- 关于「[梦境涟漪](https://milthm.com/wiki/hans/manual/features#%E6%A2%A6%E4%B9%8B%E6%B6%9F%E6%BC%AA) - 噩梦」下的表现：
  - 若已解锁「支线故事1　花裳随雨得春迟 隐藏曲1」，则可再次观看解锁隐藏曲时看到过的演出，但不会切换到隐藏曲，而是中途结束。
  - 开启后预览音源会发生变化。
  - 内部似乎被当作 Special 谱面处理，会与原谱面分开记录成绩，因此和 ???（隐藏曲）的噩梦一样被视为独立谱面。推测也因此不存在对应定数。

## 游戏相关

- Drizzle 谱面
  - 不知从哪个版本起，难度从 3 升格为 3+。
- Sprinkle 谱面
  - 定数为 9.8，但也有相当程度的虚高。
  - 全曲中会不断出现将雨露音符与雨粒音符组合而成的伪 Flick 配置，此外还有违背直觉的纵连等，不可掉以轻心。
- Cloudburst 谱面
  - 定数为 11.9。
  - 由于雨露音符会在直前交替，非交互或微纵连之类的配置在全曲中频繁出现。此外还有使用上下反转的脑力配置，若不进行一定程度的背谱将难以应对。
  - 终盘时左右摆动幅度会变得非常大，使用平板的玩家尤其需要注意。
  - 最后那处快得惊人的连打为 32 分音符。不过此处雨露的边全部为深色（保证理论值判定），因此全按也能勉强通过。
- 在「噩梦」下的表现：
  - 演出地带音源也会改变，全难度都会降下远超想象的凶暴化谱面。
  - Cloudburst 约为 12+ 或更高，Sprinkle 约为 10～10+，Drizzle 也有约 4～4+，完全可以视作完全不同的谱面。

## 谱面预览

- [Drizzle](https://storage.mhtl.im/chart/Drizzle_Sakuyahime.jpg)
- [Sprinkle](https://storage.mhtl.im/chart/Sprinkle_Sakuyahime.jpg)
- [Cloudburst](https://storage.mhtl.im/chart/Cloudburst_Sakuyahime.jpg)
- [Drizzle_Story](https://storage.mhtl.im/chart/Drizzle_Story_Sakuyahime.jpg)
- [Sprinkle_Story](https://storage.mhtl.im/chart/Sprinkle_Story_Sakuyahime.jpg)
- [Cloudburst_Story](https://storage.mhtl.im/chart/Cloudburst_Story_Sakuyahime.jpg)

