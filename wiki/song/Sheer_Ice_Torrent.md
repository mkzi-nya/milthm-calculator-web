[返回目录](./)

> 此页面正在建设中，当前为模板自动生成，部分信息可能不完整  
> 如果您愿意参与填写，可以[点此](https://github.dev/mkzi-nya/milthm-calculator-web)进行修改并提交，或联系@mkzi_nya(qq: 2450382239)
> 或[点击此处](info:download)下载对应文档并编辑后发送至[此群](https://qm.qq.com/q/3lwpuT3l8A)（qq：699731287）

# Sheer Ice Torrent

![Sheer Ice Torrent](../../jpgs/Sheer Ice Torrent.jpg)

## 曲目信息
| 曲名 | [Sheer Ice Torrent](info:info("Sheer Ice Torrent")) |
| - | - |
| 曲师 | Connor "12LBS" Skidmore |
| 曲绘 | 阿白 |
| 所属曲包 | 雨世界 |
| 更新版本 | v3.3 |
| 时长 | 02:39 |
| BPM | 116 |

## 谱面信息
| 难度 | [Drizzle](info:info("Sheer Ice Torrent", "Drizzle")) | [Sprinkle](info:info("Sheer Ice Torrent", "Sprinkle")) | [Cloudburst](info:info("Sheer Ice Torrent", "Cloudburst")) |
| - | - | - | - |
| 等级 | 2 (2.0) | 7+ (7.8) | 11 (11.1) |
| 谱师 | 姜片 | Magazet | Ariayaka |
| Note数量 | 248 | 554 | 861 |

## 解锁方法

- 阅读剧情「Journey - Seven Red Suns」。
- 再次游玩《[Bio-Engineering](info:info("Bio-Engineering"))》。

## 曲目试听

- [【Youtube】](https://youtu.be/1SvBfO1lQcs)

## 曲目相关

- 本曲为「雨世界」联动曲目。
- 在联动曲目公布帖中，本曲的曲名被隐藏了。

## 游戏相关

- 达成解锁条件后再次游玩《[Bio-Engineering](info:info("Bio-Engineering"))》时，会直接进入本曲的特殊谱面。
  - 仅首次游玩（解锁游玩）时谱面不同。且无论选择何种难度，等级均显示为 3、谱师名义也显示为「3」（所有难度都被强化为更具初见杀力的谱面）。
- 在「梦境涟漪」的「噩梦（Nightmare）」下的表现：
  - 开启该选项游玩时，可以再次游玩首次的谱面。当然 DZ、SK、CB 全部为 3。
  - 此时不会记录到游戏内的本地排行榜中，个人最佳成绩的显示则变为「噩梦」状态下游玩中的个人最佳成绩。
  - 特殊谱面在内部属于 Story 谱面，不存在难度与定数。谱师名义的「3」则为 Milthm_Team。（据官方数据库）
- CB 谱面中，所有「短雾雨音符后接雨露音符」的配置，只要意识到是微纵连、用同一根手指按下即可通过。仅此一点就能轻松不少。
  - 此外整体上重复配置较多，认真钻研谱面的话 AP 也并非遥不可及。是一张可以刷分的谱面。

## 谱面预览

- [Drizzle](https://storage.mhtl.im/chart/Drizzle_Sheer_Ice_Torrent.jpg)
- [Sprinkle](https://storage.mhtl.im/chart/Sprinkle_Sheer_Ice_Torrent.jpg)
- [Cloudburst](https://storage.mhtl.im/chart/Cloudburst_Sheer_Ice_Torrent.jpg)
- [Drizzle_Story](https://storage.mhtl.im/chart/Drizzle_Story_Sheer_Ice_Torrent.jpg)
- [Sprinkle_Story](https://storage.mhtl.im/chart/Sprinkle_Story_Sheer_Ice_Torrent.jpg)
- [Cloudburst_Story](https://storage.mhtl.im/chart/Cloudburst_Story_Sheer_Ice_Torrent.jpg)

