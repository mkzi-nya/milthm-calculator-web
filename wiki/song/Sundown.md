[返回目录](./)

> 此页面正在建设中，当前为模板自动生成，部分信息可能不完整  
> 如果您愿意参与填写，可以[点此](https://github.dev/mkzi-nya/milthm-calculator-web)进行修改并提交，或联系@mkzi_nya(qq: 2450382239)
> 或[点击此处](info:download)下载对应文档并编辑后发送至[此群](https://qm.qq.com/q/3lwpuT3l8A)（qq：699731287）

# Sundown

![Sundown](../../jpgs/Sundown.jpg)

## 曲目信息
| 曲名 | [Sundown](info:info("Sundown")) |
| - | - |
| 曲师 | James Primate |
| 曲绘 | 阿白 |
| 所属曲包 | 雨世界 |
| 更新版本 | v1.6.9 |
| 时长 | 01:26 |
| BPM | 109 |

## 谱面信息
| 难度 | [Drizzle](info:info("Sundown", "Drizzle")) |
| - | - |
| 等级 | 1 (1.0) |
| 谱师 | 179 綾吟 Kiyotsuki Attuned. |
| Note数量 | 24 |

## 解锁方法

- 在主界面选择「Rain World」后会直接开始，敬请注意。

## 曲目试听

## 曲目相关

- 本曲目为生存平台游戏《Rain World》的主菜单 BGM。
- 即使谱面结束，乐曲也会一直循环播放。
- 本游戏在分数确定时便会显示「COMPLETE」「Rhythm of Rain」等字样，因此若漏掉最后一个音符，会在音符消失之前就显示出来。
- 使用「梦境涟漪 - 噩梦」时的表现：
  - 开启该选项游玩，可以重新触发首次游玩时的演出。

## 游戏相关

- Drizzle 谱面
  - 大概是 Milthm 中最简单的谱面。
  - 只会出现判定为 Actual Perfect 的雾雨音符，因此只要连击不断就能必定达到 RHYTHM of RAIN（理论值）。

## 谱面预览

- [Drizzle](https://storage.mhtl.im/chart/Drizzle_Sundown.jpg)

